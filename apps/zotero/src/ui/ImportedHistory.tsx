...
export function ImportedHistory({ bridge, notebook_id, snapshot_id, locale }: ResearchScope) {
    ...
    return <section className="imported-history" aria-label={e.history}><h3>{e.history}</h3><p>{e.historyHelp}</p><ResearchFeedback actions={actions} locale={locale}/><button type="button" disabled={actions.locked} onClick={() => list()}>{e.refresh}</button>

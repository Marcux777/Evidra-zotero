...
export function Matrix({bridge,notebook_id,snapshot_id,locale}: {bridge:UiBridge;notebook_id:string;snapshot_id:string;locale:Locale}) {
    ...
    return <section className="matrix" aria-label={m.title}>
        {!open?<button type="button" disabled={busy} onClick={()=>{setOpen(true);void action(current=>loadForms(0,current));}}>{m.open}</button>:<>
            ...
            <div className="matrix-columns"><div className="matrix-table">{page&&<><WindowedList key={`${form.id}:${page.offset}`} items={page.items} rowHeight={140} rowKey={cellKey} label={m.title} render={cell=><>
                <button className="matrix-cell-label" type="button" disabled={locked} aria-pressed={selected!==null&&cellKey(selected)===cellKey(cell)} onClick={()=>select(cell)}><span className="matrix-cell-source" title={cell.source_title}>{cell.source_title}</span><span className="matrix-cell-separator"> · </span><strong className="matrix-cell-field" title={form.fields.find(f=>f.key===cell.field_key)?.label??cell.field_key}>{form.fields.find(f=>f.key===cell.field_key)?.label??cell.field_key}</strong></button>
            ...
            </>}/>{page.items.length===0&&<p>{m.empty}</p>}<nav className="actions" aria-label={m.title}><button type="button" disabled={locked||page.offset===0} onClick={()=>void action(async current=>{clearCell();await loadPage(form,Math.max(0,page.offset-page.limit),current);})}>{m.previous}</button><span>{page.items.length ? page.offset+1 : 0}–{page.offset+page.items.length} / {page.total}</span><button type="button" disabled={locked||page.offset+page.limit>=page.total} onClick={()=>void action(async current=>{clearCell();await loadPage(form,page.offset+page.limit,current);})}>{m.next}</button></nav></>}</div>
            ...
            <section className="bulk-review" aria-label={m.bulk}><h3>{m.bulk}: {bulk.length}</h3><p>{m.bulkLimit}</p>
                ...
                {preview&&<><h4>{m.count}: {preview.count}</h4><p>{m.bulkHelp}</p><ol>{preview.changes.map(change=><li key={change.proposal.id}><h4>{change.old.source_title} · {form.fields.find(f=>f.key===change.old.field_key)?.label??change.old.field_key}</h4>
                    <p>{m.before} · {m.revision} {change.old.revision} · {m.reviews[change.old.review_state]}</p><ValueDisplay locale={locale} value={change.old.value} unit={form.fields.find(f=>f.key===change.old.field_key)?.unit??null}/><p>{m.after} · {m.values[change.proposal.value_state]}</p><ValueDisplay locale={locale} value={change.proposal.value} unit={form.fields.find(f=>f.key===change.proposal.field_key)?.unit??null}/>

...
export function ProposalReview({cell,field,locale,busy,readOnly,proposals,history,hits,onSearch,onInspect,onVisual,onPropose,onDecide,onPage,onHistory,onBulk}: {
    ...
    return <section className="proposal-review" aria-label={t.proposals}>
        ...
        {selected&&<div className="selected-proposal" key={selected.id}>
            <p>{selected.origin === 'EXTERNAL_CLIENT' ? catalog(locale).mcp.origin : selected.origin === 'HUMAN_CLIENT' ? t.origin : selected.origin === 'MODEL_RUN' ? catalog(locale).jobs.modelOrigin : catalog(locale).jobs.measuredOrigin} · {selected.principal}</p>
            {selected.origin === 'EXTERNAL_CLIENT' && <p>{catalog(locale).mcp.declared}: {selected.declared_model ?? catalog(locale).mcp.unknownModel}</p>}

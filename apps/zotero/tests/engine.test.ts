...
test('controller requires current consent, validates receipt, hides credential, heartbeats and stops only owned process', async () => {
    ...
    const requiredRoutes: ['GET' | 'POST' | 'PUT' | 'DELETE', string][] = [
        ...
        ['POST', `${snapshot}/documents/text-view`],
        ['POST', `${snapshot}/documents/original-view`],
    ...
    const rejectedRoutes: [string, string][] = [
        ...
        ['GET', `${snapshot}/documents/text-view`], ['PUT', `${snapshot}/documents/text-view`],
        ['POST', `${snapshot}/documents/text-view?offset=0&limit=50`],
        ['POST', `${snapshot}/documents/text-view/extra`], ['POST', `${snapshot}/documents/text%2dview`],
        ['GET', `${snapshot}/documents/original-view`], ['PUT', `${snapshot}/documents/original-view`],
        ['POST', `${snapshot}/documents/original-view?unit_index=0`], ['POST', `${snapshot}/documents/original-view/extra`],

...
test('Sources requires a current preview, preserves filter drafts on notifications and exposes labeled opt-in controls', async () => {
    ...
    const bridge = { request: async (message: UiMessage) => {
        ...
        if (message.op === 'sources.preview') {
            lastPreview = { preview: { id: 'b'.repeat(32), stage_id: 'e'.repeat(32), notebook_id: notebook.id, expected_revision: 1, selection: message.selection, items: [{ identity: { profile_instance_id: 'p1', library_id: 1, item_key: 'PAPER001' }, id: 'c'.repeat(64), version_id: 'v1', version: '1', title: 'Selected paper', year: 2020, year_state: 'known', item_type: 'journalArticle', tags: [], doi: null, remote_library_id: null, remote_group_id: null, contents: [] }], removed: [], added: ['c'.repeat(64)], dropped: [], changed: [], possible_duplicates: [] }, unavailable: [] };

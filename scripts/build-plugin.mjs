...
const { values } = parseArgs({options:{output:{type:'string'},stage:{type:'string'}}});
const stamp=new Date().toISOString().replace(/[:.]/g,'-');
const stage=values.stage?resolve(values.stage):resolve('.local/task-2/build',stamp);const out=resolve(values.output??'.local/task-2/dist');
...
const licenses=[],noticePackages=[];
// The bundler inputs, not a manually maintained dependency list, determine notices.
// Standalone Ajv-generated validators also contain Ajv runtime code.
const packageRoots=new Set([resolve('node_modules/ajv')]);
for(const input of [...Object.keys(native.metafile.inputs),...Object.keys(renderer.metafile.inputs)]){
  if(!input.replaceAll('\\','/').includes('node_modules/'))continue;
  let root=dirname(resolve(input));
  while(!existsSync(join(root,'package.json'))){const parent=dirname(root);if(parent===root)throw new Error(`No owning package: ${input}`);root=parent;}
  packageRoots.add(root);
}
for(const root of [...packageRoots].sort()){
  const pkg=JSON.parse(await readFile(join(root,'package.json'),'utf8'));
  const files=(await readdir(root)).filter(n=>/license|licence|copying|notice/i.test(n));if(!files.length)throw new Error(`Missing license: ${pkg.name}`);
  const texts=await Promise.all(files.map(n=>readFile(join(root,n),'utf8')));
  licenses.push(`${pkg.name} ${pkg.version}\n${texts.join('\n')}`);
  noticePackages.push({name:pkg.name,version:pkg.version,license:pkg.license,files:files.map((name,i)=>({name,sha256:createHash('sha256').update(texts[i]).digest('hex')}))});
}
...
await writeFile(join(out,'build-receipt.json'),JSON.stringify({created_at:new Date().toISOString(),stage,xpi,sha256:createHash('sha256').update(await readFile(xpi)).digest('hex'),manifest,notices:noticePackages,native: native.metafile,renderer:renderer.metafile,native_acceptance:'NOT_VERIFIED'},null,2));
console.log(`XPI: ${xpi}`);
